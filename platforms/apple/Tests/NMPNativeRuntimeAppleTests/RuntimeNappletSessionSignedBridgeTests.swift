...
final class RuntimeNappletSessionSignedBridgeTests: RuntimeNappletSessionTestCase {
    ...
    func testSignedGoodMorningExecutesAuthoredBridgeTrafficInTrustedView() async throws {
        ...
        // The published fixture's own manifest declares no `requires` tags,
        // and no runtime code special-cases its identity into a capability
        // profile, so its permission review is empty. The capabilities this
        // bridge exercise needs are granted explicitly here instead.
