...
### Verdict authority must be typed

A verdict may replace underlying authority on the primary path only when the
owning Rust or NMP boundary projects an exhaustive typed verdict or consent
summary for the exact action. Native may map typed variants to language; it
must not parse opaque JSON or strings, infer Nostr/event, delivery, policy,
platform, or completeness semantics, or manufacture success. If that typed
projection does not exist, the exact authority or consent material remains on
the primary path even when it is technical or deliberately hostile.

Collapsed evidence supplements a typed verdict; it never substitutes for
information required to understand or authorize the action. Unknown,
unavailable, or unrepresentable state blocks/refuses or remains explicitly
unavailable; it never becomes success. A terminal action refusal is
`.blocked`, not `.caution`.
...
The sole raw-domain exception is a runtime-projected capability domain with no
entry in the pinned native vocabulary. It appears as unrecognised so consent
never hides authority. A runtime fallback title is evidence, not trusted
friendly vocabulary.
...
phrased as what the napplet will be able to do. For a fully decidable review,
allow/deny switches are visible by default. Switching one on uses the runtime's
own `recommendedDecision`; an untouched common action applies every runtime
recommendation in one gesture. Native never chooses grant scope.

Mixed or all-managed reviews are the deliberate exception. The current Rust
contract requires a decision for every requested domain while rejecting a
user-supplied decision for a managed domain. Native therefore shows the full
review but disables submission instead of fabricating a managed decision or
submitting a partial batch. That remains blocked until the Rust contract can
accept the review atomically.
...
The sole non-alarm exception is `PendingWriteConsent`. It is a typed, bounded
exact-authority state requiring an immediate publish-or-refuse decision.
Caution colour may reinforce that consent boundary only when the complete
event and both decisions remain explicit in words. This does not let native
derive a warning from payload content or reuse caution colour for general
status.
...
domain is shown under the sole exception in §1. The runtime's fallback title
does not become trusted friendly vocabulary. Unknown is a verdict too, and it
is the one an ordinary person most needs stated plainly.
