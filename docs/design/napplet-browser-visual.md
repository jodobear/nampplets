...
The design criteria in §1–§9 came from reading the implementation, including
`NappletStyle.swift`, `NappletDisclosure.swift`,
`CatalogView.swift`, `CatalogEntryRow.swift`, `CatalogInstallReviewSheet.swift`,
`PermissionReviewSheet.swift`, and `CatalogBrowseEvidenceView`; and from
sampling
`NappletVocabulary.swift`, `NappletIdentityPresentation.swift`,
`CatalogModels.swift`, and `WorkbenchLibraryBuildRow.swift`.

This document intentionally does not freeze source-token counts. They become
false as soon as implementation moves. Instead, each implementation slice must
audit the exact tree it changes against the durable rules below: no decorative
verification seals, a deliberately small icon vocabulary, semantic colour only
when it carries a typed verdict, leading-aligned empty states, and continuous
corner geometry wherever the shell draws a rounded surface.
...
are not asserted in prose; executable thresholds consume the production RGB
tokens in `NappletInkContrastTests`.
...
**Every shell-authored rounded rectangle uses `style: .continuous`.** This is
not a preference; iOS and macOS draw squircles and circular-arc cards sit
visibly apart from adjacent system geometry.
...
1. **The accent**, on **one** element per screen: the primary action. Nothing
   ...
   icons.
2. **Caution and refusal inks**, which appear when a projected
   `NappletTrustVerdict` is `.caution` or `.blocked`. The caution ink also marks
   `PendingWriteConsent`: a typed, bounded exact-authority state requiring an
   immediate publish-or-refuse decision. Native never derives either state.
...
`Color(light:dark:)`. `NappletInkPalette` owns the exact sRGB values;
`NappletInkContrastTests` consumes those same values rather than mirroring them.

**Ink ramp**

| Token | Tested minimum on `paper`, light and dark | Use |
| --- | --- | --- |
| `ink` | 7 : 1 | All primary text |
| `inkSecondary` | 4.5 : 1 | Supporting sentences, labels |
| `inkTertiary` | 3 : 1 | **Non-essential only** (see below) |

**Ground**

| Token | Use |
| --- | --- |
| `paper` | Window / page ground and contrast-test background |
| `paperRaised` | Sheets, popovers, iOS grouped ground |
| `fillQuiet` | Cards, search field, open evidence block |
| `fillSelected` | Sidebar selection, pressed states |
| `rule` | Hairline dividers |

**Accent — one hue, chosen by elimination**

| Token | Executable threshold, light and dark |
| --- | --- |
| `accent` | 4.5 : 1 on `paper` |
...
hue in the space that is not already carrying a semantic. The desaturated
production token moves decisively off systemBlue while staying sober. It reads
as ink from a good pen, which is the correct association for a product about
signatures.

**Semantic — rare by construction**

| Token | Executable threshold, light and dark | Use |
| --- | --- | --- |
| `caution` | 4.5 : 1 on `paper` | `.caution` glyph + text |
| `refusal` | 4.5 : 1 on `paper` | `.blocked` glyph + text |
| semantic ground | not a text carrier | Token at 10% behind `NappletNotice` |

### 4.3 Three colour defects this system excludes

1. **System `.orange` and `.red` in `NappletNotice`.** Their resolved values
   vary by platform and appearance, so one prose ratio would be false
   precision. Use the production-owned `caution` / `refusal` tokens whose light
   and dark thresholds are executable.
2. **Direct semantic colour on plain rows.** When a state is already fully
   legible as a sentence, per ADR 0008 §4, colour there is
   reinforcing nothing that the words have not said, and it is what makes the
   list look like a status board. **Ruling: those rows do not get colour.** They are
...
3. **`.tertiary` carrying an only-source publisher line.** It is held only to
   an explicit 3 : 1 product floor, not the 4.5 : 1 body-text threshold, and
   the publisher is not decoration — in a
...
The icon vocabulary is an owned, reviewed surface, not an incidental sum of
whatever symbols individual views chose. The target is **under 20**, and the
rules that get us there are these.
...
6. **Never an icon in an empty state.** §7.2. Centred, icon-led empty-state
   chrome is the pattern to remove.
...
9. **Never an icon carrying `accent` or a semantic colour**, except the two
   verdict glyphs and the typed `PendingWriteConsent` indicator. Other icons are
   `inkSecondary`.
...
| `checkmark` on "doesn't ask for access to anything" | The sentence is the verdict. A checkmark beside it is a green seal in greyscale clothing. |
...
3. **The place has a header that does work.** `place` title ("Napplets") plus
   one true `lede` line — *"From the latest lookup on this device."* A page with a
...
5. **Centred `ContentUnavailableView` is refused on primary paths.** It is
   centred,
...
| Attribution | *"Ana"* `caption`; no aggregate count | named author | named author |
...
┌────────────────┬──────────────────────────────────────────────────────────┐
│                │                                                          │
│  NAPPLETS      │   [🔍 Filter these napplets                        ]     │  ← fillQuiet, radiusSmall
│                │                                                          │
│  ▸ Discover    │                                                          │  ← page 64
│    Saved       │   Napplets                                               │  ← place, New York 26 semibold
│    Yours       │   From the latest lookup on this device.                 │  ← lede, inkSecondary
│                │                                                          │  ← spacious 48
│                │   ┌──────────────────────────────────────────────────┐   │
...
│                              │
│  [🔍 Filter these napplets]  │
│                              │
│  From the latest lookup on   │  ← lede
│  this device.                │
│                              │
│  ┌────────────────────────┐  │
...
│                │   │                                                  │   │  ← comfortable 16
│                │   │  The napplet lists these as optional             │   │  ← caption semibold, inkSecondary
│                │   │  ⊙  Send notifications                           │   │
│                │   └──────────────────────────────────────────────────┘   │
│                │   Adding doesn't grant access or open the napplet.       │  ← secondary, inkSecondary
...
│                │   │     It does one thing.                           │   │
│                │   │     Ana Ruiz                                     │   │  ← caption attribution; no count
│                │   └──────────────────────────────────────────────────┘   │
│                │                                                          │  ← generous 32
...
│  └──────────────────────────────────────┘  │
│                                            │
│  Adding doesn't grant access or open it.   │  ← secondary, inkSecondary
│  Access is reviewed separately.            │
│                                            │  ← spacious 48
│  Technical details  ›                      │
└────────────────────────────────────────────┘
...
```
┌────────────────────────────────────────────┐
│  Not Now          Use Recommended Settings │  ← confirmation action, accent
├────────────────────────────────────────────┤
│                                            │
...
For a fully decidable review, allow/deny switches are visible immediately.
Switching one on applies Rust's `recommendedDecision`; native does not choose
or rank scopes. If no switch is edited, the common confirmation action applies
every Rust recommendation in one gesture. A review containing any managed
capability remains visible but non-confirmable until Rust can accept that exact
review atomically.
...
optional `caption` "Written about an earlier version." Card, `comfortable` 16.
...
`"\(name) wrote: \(text)"`, followed by the version note if present. The tier
heading is `.isHeader`. **The tier itself is never
...
Replaces centred `ContentUnavailableView` on primary product paths.
...
The "Filtering the napplets shown here." line (experience doc
...
| **Centred, icon-led empty-state chrome** | §7.2. It turns the primary state of a young catalog into failure chrome. |
...
| **Semantic colour without a typed verdict** | §4.3. A legible sentence needs no diagnostic tint unless colour carries additional, authoritative meaning. |
...
| **Symbols outside the reviewed vocabulary** | §6. Nobody learns a sprawling icon vocabulary for an app they open twice a week. |
...
**12.2 §7.1's bounded latest window and my count-driven column rule interact.**
At 40+ items, a single scroll of full-measure cards is very long. The
two-column switch at ≥7 helps presentation, but it does not alter the finite
window contract. Capacity, `hasMore`, source shortfalls, and omitted rows stay
observable; an explicit bounded load-more/page action is preferable to
presenting a long finite window as the entire inventory.
...
1. **Tokens.** Extend `NappletMetrics` (§3.1); add the `NappletInk` palette
   (§4.2); use `.continuous` for shell-authored radii. No layout changes. Immediately
...
3. **Colour rules.** Audit semantic-colour call sites against §4.3; fix
...
5. **`NappletEmptyPlace`** replacing centred empty-state chrome (§10.9).
