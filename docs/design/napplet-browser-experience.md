...
- Scope: browse, search, the napplet page, the library, and the bounded social
  layer (saving and attributed reviews). Visual design is deliberately out of scope;
...
**Discover** — the latest bounded catalog window, arranged. Not ranked, not
partitioned. Ordered newest-first (§7). Its job is to show the current
replacement honestly: what the latest lookup on this device returned, never a
claim about the complete network inventory.
...
> *Filtering the napplets shown here.*
...
That is a verdict we can stand behind because it is a fact about which event
id one review's tag names. The app does not aggregate those labels into
"most reviews" or any other section-level claim.
...
weaker: *"People in your wider network wrote about this."* Same query one hop
further out.
...
Within every tier, order is **newest first**. It is predictable and explainable
in four words, but it is not attack-resistant: a cheap flood can crowd a finite
newest-first window. Rust must therefore own finite per-tier and per-author
bounds, event-id deduplication, deterministic tie-breaking, and observable
omission/fairness evidence. Native renders those decisions and never invents
its own sampling or ranking.
...
Removing the scoreboard removes one cheap way to manufacture authority; it
does not make the list sybil-immune. A thousand hostile reviews can still
crowd out ten useful ones in a finite newest-first window. The Rust-owned
per-tier/per-author bounds, deduplication, and observable omission/fairness
facts above are required before this surface is supportable.
...
## 6. Saving and consent

### 6.1 Reactions: out of scope

Reactions are not part of this runtime plan, either on napplets or on reviews.
They are easy to turn into an implied popularity score and add a protocol
dependency without helping someone understand, install, or safely run a
napplet. Attributed written reviews may be evaluated separately; reaction
events are neither a requirement nor a planned dependency for this train.
...
1. **Recency.** Discover's default order is newest manifest first. It is not a
   quality signal and never claims to be. It is predictable, it makes a live
   network feel alive, and gaming it buys you visibility rather than
   endorsement — a much smaller prize.
...
- **Discover shows one bounded latest window.** No horizontal carousels.
  Capacity, `hasMore`, source shortfalls, and omitted rows remain observable;
  a finite page must never present itself as the complete network inventory.
...
- **"Still arriving" is a state, not a spinner.** The catalog is a live window
  ...
  preserved.
- **Never imply completeness.** The footer distinguishes a proven capacity or
  `hasMore` condition from source failure. A shortfall proves incomplete
  acquisition; it does not prove additional rows exist.

---

## 8. Signed out, and the account ask

Public browsing, local filtering, Tier 3 reviews, adding, running, and saving
work with no account. This falls out of the architecture rather than being
engineered — catalog queries are public (`SourceAuthority::Public` /
`AccessContext::Public`) and saves are local.
...
| Reactions on napplets or reviews | §6.1 — not a runtime requirement or planned dependency |
...
- The IA change: window/tab structure, three destinations, the napplet
...
- Sparse-catalog behaviour for one bounded latest window: honest empty states,
  local-filter wording, and reusing `CatalogBrowseEvidenceView` for off-path
  evidence.
...
### 10.2 Needs Apple model and projection work

- **Discover ordered by recency.** The FFI catalog entry carries `created_at`,
  but the Apple `CatalogEntry` model and Workbench projection currently drop
  it and perform no recency sort. Add the field, copy it through the Apple
  projection, and sort with a deterministic tie-breaker. This requires no Rust
  change, but it is not implemented today.

### 10.3 Needs Rust work in this repo (NMP facade is already sufficient)
...
### 10.4 Needs protocol work, or an ownership ruling this repo has not made
...
- **A note on hand-composition.** The facade re-exports `UnsignedEvent`,
  ...
  hand-compose kind 30003 events and publish them. **It should not.**
...
### 10.5 Things I am guessing about
