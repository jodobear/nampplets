...
struct CatalogBrowseEvidenceView: View {
    ...
    /// Separates a proven display bound from acquisition shortfalls. A source
    /// failure says the result is incomplete; it does not prove more rows
    /// exist.
    var summary: String {
        ...
        if isStillLooking {
            return count == 0
                ? "Still looking…"
                : "\(count) \(noun) so far — still looking…"
        }
        switch (hasMoreRows, hasUnavailableSources) {
        case (true, true):
            return "\(count) \(noun) shown — more are available; "
                + "some sources were unavailable"
        case (true, false):
            return "\(count) \(noun) shown — more are available"
        case (false, true):
            return "\(count) \(noun) — some sources were unavailable"
        case (false, false):
            return "\(count) \(noun)"
        }
    }
    ...
    private var isStillLooking: Bool {
        evidence.window == .requesting
            || evidence.sourceEvidence.contains { source in
                switch source.status {
                case .requesting, .connecting, .awaitingAuthentication: true
                case .disconnected, .authenticationDenied, .error: false
                }
            }
    }

    private var hasUnavailableSources: Bool {
