...
enum NappletEvidenceCopy {
    static let accessibilityHint =
        "Shows the exact technical details behind this screen"
}
...
struct NappletEvidence<Content: View>: View {
    ...
    var body: some View {
        ...
        .accessibilityHint(NappletEvidenceCopy.accessibilityHint)
    }
}
...
struct NappletField: Equatable, Hashable, Sendable {
    let label: String
    let value: String
...
struct NappletFieldOccurrence: Identifiable, Equatable, Sendable {
    let id: Int
    let field: NappletField
}
...
struct NappletFieldGrid: View {
    ...
    var occurrences: [NappletFieldOccurrence] {
        fields.enumerated().map { offset, field in
            NappletFieldOccurrence(id: offset, field: field)
        }
    }
