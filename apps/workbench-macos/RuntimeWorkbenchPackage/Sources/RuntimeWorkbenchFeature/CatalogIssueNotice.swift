...
struct CatalogIssueNotice: View {
    enum Intent: Equatable, Sendable {
        ...
        var sentence: String {
            switch self {
            case .browsePartial:
                "Some napplets couldn't be loaded."
            case .browseBlocked:
        ...
        var verdict: NappletTrustVerdict {
            ...
            case .browsePartial:
                .caution(sentence)
            case .browseBlocked, .resolveBlocked, .installBlocked:
                .blocked(sentence)
            }
        }

        var isBrowse: Bool {
            self == .browsePartial || self == .browseBlocked
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: NappletMetrics.tight) {
            NappletNotice(verdict: presentation.intent.verdict)

            NappletEvidence(label: "Details") {
                NappletFieldGrid(fields: [
                    NappletField(
                        presentation.issue.title,
