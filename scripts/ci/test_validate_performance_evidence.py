...
def reference(value: dict) -> dict:
    return {
        ...
        "source_revision": value["build"]["source_revision"],
        "source_provenance": value["build"]["source_provenance"],
...
class PerformanceEvidenceTests(unittest.TestCase):
    ...
    def test_valid_result_fixture_is_accepted(self) -> None:
        fixture = MODULE_PATH.parents[2] / "performance/fixtures/result-v1.valid.json"
        report = VALIDATOR.validate_bytes(fixture.read_bytes())
        self.assertTrue(report["ok"], report)
        self.assertEqual(report["kind"], "result")
    def test_schemas_bound_all_instance_objects_strings_and_arrays(self) -> None:
        for schema_id, name in VALIDATOR.SCHEMA_FILES.items():
            root = json.loads((VALIDATOR.SCHEMA_DIR / name).read_text())
            self.assertEqual(root["$id"], schema_id)
            pending = [root]
            while pending:
                node = pending.pop()
                if isinstance(node, dict):
                    if node.get("type") == "object":
                        self.assertFalse(node.get("additionalProperties", True), node)
    ...
    def test_canonical_json_and_duplicate_members_are_enforced(self) -> None:
        ...
        self.assert_error(b'{"schema_id":"\\ud800"}', "invalid_json")
        self.assert_error(b"[" * 2000 + b"]" * 2000, "root_not_object")
    def test_parser_and_runner_ceilings_have_typed_codes(self) -> None:
        ...
        value = result_artifact()
        value["samples"] = [sample(index, "success", 1) for index in range(10001)]
        self.assert_error(value, "sample_limit_exceeded")
    ...
    def test_observed_only_requires_not_evaluated_confidence(self) -> None:
        ...
        comparison["producer_summary"] = VALIDATOR.summarize_comparison(
            comparison["baseline"]["identity"], comparison["candidate"]["identity"],
            comparison["confidence"])
        comparison["checksum_sha256"] = VALIDATOR.compute_artifact_checksum(comparison)
        self.assertTrue(VALIDATOR.validate_bytes(encoded(comparison))["ok"])
