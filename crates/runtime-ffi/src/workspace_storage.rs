...
pub(crate) enum StoredWorkspaceAxis {
    Horizontal,
    Vertical,
}
...
#[derive(Clone, Copy, Debug, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub(crate) enum StoredWorkspaceRole {
    Feed,
    Detail,
    Profile,
    Thread,
...
#[derive(Clone, Copy, Debug, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub(crate) enum StoredWorkspaceRenderer {
    Native,
    LegacyNapplet,
    Surface,
    Unavailable,
...
pub(crate) struct StoredWorkspaceSlotV1 {
    slot_id: String,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct StoredWorkspaceV1 {
    schema_version: u16,
...
pub(crate) fn workspace_from_parts(
    workspace_id: &str,
...
pub(crate) fn validate_workspace_handler(slot: &RuntimeWorkspaceSlot) -> Result<(), String> {
    match slot.renderer {
...
pub(crate) fn validate_hex64(field: &str, value: &str) -> Result<(), String> {
...
pub(crate) fn parse_workspace_object(field: &str, raw: &str) -> Result<serde_json::Value, String> {
...
pub(crate) fn stored_axis(axis: RuntimeWorkspaceAxis) -> StoredWorkspaceAxis {
    match axis {
...
pub(crate) fn ffi_axis(axis: StoredWorkspaceAxis) -> RuntimeWorkspaceAxis {
    match axis {
...
pub(crate) fn stored_role(role: RuntimeWorkspaceRole) -> StoredWorkspaceRole {
    match role {
...
pub(crate) fn ffi_role(role: StoredWorkspaceRole) -> RuntimeWorkspaceRole {
    match role {
...
pub(crate) fn stored_renderer(renderer: RuntimeWorkspaceRenderer) -> StoredWorkspaceRenderer {
    match renderer {
...
pub(crate) fn ffi_renderer(renderer: StoredWorkspaceRenderer) -> RuntimeWorkspaceRenderer {
    match renderer {
