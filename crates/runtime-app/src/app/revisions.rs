...
use super::AppState;
use crate::views::{AppSnapshot, SectionRevisions};
...
pub(super) fn advance_revisions(
    ...
    fn step(previous: u64, changed: bool) -> u64 {
        if changed {
    ...
    SectionRevisions {
        library: step(revisions.library, current.library != previous.library),
        sessions: step(
            revisions.sessions,
        ...
        bindings: step(revisions.bindings, current.bindings != previous.bindings),
        pending_writes: step(
            revisions.pending_writes,
            current.pending_writes != previous.pending_writes,
        ),
        receipts: step(revisions.receipts, current.receipts != previous.receipts),
        workspaces: step(
            revisions.workspaces,
            current.workspaces != previous.workspaces,
        ),
        resources: step(revisions.resources, current.resources != previous.resources),
